...
The dashboard portals also own same-origin Route Handlers under `src/app/api`. Their sign-in forms post credentials to `/api/auth/session`; the handlers translate those requests into handwritten `@template/api-client` operations and store access and refresh tokens in app-specific HttpOnly cookies. The User Portal profile client routes `PUT /api/v1/stakeholders/me/profile` to its local `/api/profile` handler, which reads the access-token cookie server-side and forwards the update to the backend. The Admin sign-in handler additionally requires the configured role claim before it writes cookies. Logout controls call the session endpoint with `DELETE`; each handler attempts backend termination and expires both portal cookies even when that backend session has already ended. Each dashboard's `src/proxy.ts` protects page routes before rendering, rotates a refresh-only session through the backend, and preserves the local destination when redirecting to that portal's public sign-in page. `src/lib/server-api.ts` creates a request-scoped server client, forwards only selected tracing metadata, reads the access token server-side, and defaults authenticated requests to `cache: "no-store"`. Browser components never import the server entry point or receive backend tokens. Public, CORS-approved calls continue directly through the browser-safe `src/lib/api.ts` adapter; authenticated mutations are routed to explicit same-origin handlers.
