...
Validation starts after a field is touched and updates as the user corrects it. Native browser validation is disabled so messages have one predictable source. Fields associate labels, hints, and errors with the input, expose `aria-invalid`, announce errors, and provide an accessible password visibility control. `SearchableSelectField` uses an input and datalist for type-ahead discovery while storing the selected option's backend value in the form. The submit button uses both React Hook Form's `isSubmitting` and an optional mutation pending state to prevent duplicate submissions.
