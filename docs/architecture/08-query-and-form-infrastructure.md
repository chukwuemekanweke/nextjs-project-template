...
Validation starts after a field is touched and updates as the user corrects it. Native browser validation is disabled so messages have one predictable source. Fields associate labels, hints, and errors with the input, expose `aria-invalid`, announce errors, and provide an accessible password visibility control. `SearchableSelectField` renders an anchored, keyboard-operable type-ahead list while storing the selected option's backend value in the form. `ConfirmationCodeField` presents one form value as six focus-managed inputs and supports typing, paste, one-time-code autofill, arrow navigation, and backspace navigation. When a paste exactly fills the configured code length, its optional completion callback lets the owning workflow run its normal validated submit path. The submit button uses both React Hook Form's `isSubmitting` and an optional mutation pending state to prevent duplicate submissions.
