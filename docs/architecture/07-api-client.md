...
`packages/api-client` owns base-URL configuration, Fetch execution, JSON and multipart serialization, query/path encoding, authentication headers, correlation IDs, cancellation, response parsing, normalized errors, and domain operations. It has no React or TanStack Query dependency. Domain clients group operations for authentication, payments, profiles, providers, and reference data. Authentication includes email-existence checks, registration, email confirmation, confirmation-code requests, and authenticated password changes as handwritten operations. Registration and code-request responses retain the backend `retryAtUtc` value so applications can enforce the server-defined resend cooldown. Because successful email confirmation now returns session tokens, applications invoke that operation only from a server-side BFF boundary; `@template/api-react` does not expose a browser confirmation mutation. Password changes also cross an app-owned BFF Route Handler so current and new passwords are sent with the HttpOnly-authenticated server request and are never placed in query data. Inbound webhook endpoints are deliberately excluded because they are server-to-server callbacks, not browser operations.
